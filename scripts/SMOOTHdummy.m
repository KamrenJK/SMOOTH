function stat = SMOOTHdummy(cfg, varargin)

% SMOOTHdummy - surrogate map-based group-level stats with RSF. ONLY used
% for comparison in the paper (see fig 5), not intended for actual analyses.
%
% Use as:
%   stat = SMOOTHdummy(cfg, source1, source2, ...)
%
% Output (stat)
...
for s = 1:nSubjects
  ...
  [mapL, ~, ~, e2m_cache] = elec2map_build('L', cfg, lh, sph_l, pos, dv, sigma_mm,  hemi, e2m_cache);
  [mapR, ~, ~, e2m_cache] = elec2map_build('R', cfg, rh, sph_r, pos, dv, sigma_mm, ~hemi, e2m_cache);
...
% -------------------------------------------------------------------------
% Steps 2: Group-Level Cluster Statistics
% -------------------------------------------------------------------------
...
for iter = 1:cfg.numrandomization+1
  if iter <= cfg.numrandomization
    ...
    for s = 1:nSubjects
      c = 2.*CoinFlip(1,0.5)-1; 
      permMaps(:,s) = allmaps(:,s).*c;
      if strcmp(cfg.keepsurrogates, 'yes')
